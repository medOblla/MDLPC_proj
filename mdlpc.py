...
class MDLP_Discretizer(TransformerMixin):
    def __init__(self, features=None, raw_data_shape=None):
        ...
        if features is None:  # Assume all columns are numeric and need to be discretized
            ...
            if np.issubdtype(features.dtype, np.integer):
                ...
                if raw_data_shape is None:
                    raise Exception(
                        'If features is a boolean array, raw_data_shape must be != None')
                ...
                raise Exception(
                    'features argument must a np.array of column indices or a boolean mask')

    def fit(self, X, y):
        self._data_raw = X  # copy of original input data
        # make sure class labels is a column vector
        self._class_labels = y.reshape(-1, 1)
        ...
        if len(self._col_idx) != self._data_raw.shape[1]:
            self._ignore_col_idx = np.array([var for var in range(
                self._data_raw.shape[1]) if var not in self._col_idx])
    ...
    def feature_boundary_points(self, values):
        ...
        unique_vals = np.unique(data_partition[:, 0])
        ...
        for i in range(1, unique_vals.size):
            previous_val_idx = np.where(
                data_partition[:, 0] == unique_vals[i-1])[0]
            current_val_idx = np.where(
                data_partition[:, 0] == unique_vals[i])[0]
    ...
    def compute_boundary_points_all_features(self):
        ...
        boundaries[:, self._col_idx] = np.apply_along_axis(
            padded_cutpoints_array, 0, self._data_raw[:, self._col_idx], self._data_raw.shape[0])
    ...
    def boundaries_in_partition(self, X, feature_idx):
        ...
        mask = np.logical_and(
            (self._boundaries[:, feature_idx] > range_min), (self._boundaries[:, feature_idx] < range_max))
    ...
    def best_cut_point(self, X, y, feature_idx):
        ...
        if candidates.size == 0:
            return None
        gains = [(cut, cut_point_information_gain_numpy(X, y, cut_point=cut))
    ...
    def generate_bin_descriptions(self):
        ...
        for attr in self._col_idx:
            if len(self._cuts[attr]) == 0:
                ...
                bin_labels = ['%s_to_%s' % (str(cuts[i]), str(
    ...
    def apply_cutpoints(self, data):
        ...
        for attr in self._col_idx:
            if len(self._cuts[attr]) == 0:
                ...
                cuts = [-np.inf] + self._cuts[attr] + [np.inf]
                discretized_col = np.digitize(
                    x=data[:, attr], bins=cuts, right=False).astype('float') - 1
